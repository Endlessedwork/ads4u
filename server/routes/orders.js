...
router.post('/api/orders', requireAuth, async (req, res) => {
  ...
  try { new URL(link); } catch { return res.status(400).json({ error: 'Invalid link URL' }); }
  ...
  try {
    ...
    const row = db.prepare(`INSERT INTO orders (user_id, ads4u_order_id, service_id, service_name, link, quantity, charge) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(req.user.id, String(result.order), serviceId, serviceName || `Service #${serviceId}`, link, qty, result.charge || null);
    res.json({ id: row.lastInsertRowid, ads4u_order_id: String(result.order), charge: result.charge });
  } catch (err) {
    console.error('Failed to place order:', err.message);
    if (err.code === 'ADS4U_API_ERROR') return res.status(502).json({ error: err.message });
...
router.get('/api/orders', requireAuth, (req, res) => {
  ...
  let whereClause = '';
  const params = [];
  if (req.user.role !== 'admin') { whereClause = 'WHERE o.user_id = ?'; params.push(req.user.id); }
  if (status) { whereClause += whereClause ? ' AND o.status = ?' : 'WHERE o.status = ?'; params.push(status); }
  const countRow = db.prepare(`SELECT COUNT(*) as total FROM orders o ${whereClause}`).get(...params);
  const orders = db.prepare(`SELECT o.*, u.name as user_name, u.email as user_email FROM orders o JOIN users u ON u.id = o.user_id ${whereClause} ORDER BY o.created_at DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
...
router.get('/api/orders/:id/status', requireAuth, async (req, res) => {
  ...
  if (req.user.role !== 'admin' && order.user_id !== req.user.id) return res.status(403).json({ error: 'Access denied' });
  try {
    const result = await getOrderStatus(order.ads4u_order_id);
    db.prepare(`UPDATE orders SET status = ?, charge = ?, remains = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(result.status, result.charge, result.remains, order.id);
    res.json({ id: order.id, status: result.status, charge: result.charge, remains: result.remains, start_count: result.start_count, currency: result.currency });
  } catch (err) {
...
router.get('/api/orders/:id/refill-status', requireAuth, async (req, res) => {
  ...
  if (!refillId) return res.status(400).json({ error: 'refillId query parameter is required' });
  try {
